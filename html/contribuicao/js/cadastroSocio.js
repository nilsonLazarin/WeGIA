        ...
        if (data == 0 || data.includes("false")) {
            ...
            if (dados[0] == "SEM_ENDERECO") {
                ...
                if (formaContribuicao == 'pix') {
                    ...
                    $("#gerar_boleto").click(function () { geraFormaContribuicao(); });
                }
...
function editar_informacoes() {
    ...
    $.post("../php/editaSocio.php", { 'nome': nome, 'telefone': tel, 'email': email, 'doc': doc, 'datanascimento': data_n, 'cep': cep, 'log': rua, 'numero': numero, 'comp': compl, 'bairro': bairro, 'cidade': cidade, 'uf': uf, 'tipoPessoa': tipoPessoa }).done(function (data) {
        ...
        $("#pag3").hide();
        $("#dados_atualizados").click(function () {

            const formaContribuicao = $("#forma-contribuicao").val();

            if(formaContribuicao == "pix"){
                $("#form2").html('<h3>Obrigado por contribuir mais uma vez, ' + nome + '!</h3><br>Clique em "EMITIR QRCODE" e aguarde o redirecionamento.<br><div class="container-contact100-form-btn"><button class="contact100-form-btn" id = "emitir_qrcode" onClick="setLoader(this)"><i style="margin-right: 15px; " class="fa fa-long-arrow-right m-l-7"aria-hidden="true"></i>EMITIR QRCODE</button></div><div style="display: none" class="container-contact100-form-btn"><span class="contact100-form-btn" id = "editar_infos"><i style="margin-right: 15px; " class="fa fa-long-arrow-left m-l-7"aria-hidden="true"></i>EDITAR DADOS CADASTRADOS</span></div><div class="container-contact100-form-btn"><span class="contact100-form-btn"><i style="margin-right: 15px; " aria-hidden="true"></i><a class= "botao" href="../index.php">VOLTAR A PÁGINA INICIAL</a></span></div>');
                $("#emitir_qrcode").click(function () { geraFormaContribuicao(); });
            }else{
