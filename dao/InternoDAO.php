...
class InternoDAO
{
    ...
    public function listarTodos(){
        ...
        try{
            $internos=array();
            $pdo = Conexao::connect();
            $consulta = $pdo->query("SELECT p.nome,p.cpf,i.id_interno FROM pessoa p INNER JOIN interno i ON p.id_pessoa = i.id_pessoa");
            $produtos = Array();
            $x=0;
            while($linha = $consulta->fetch(PDO::FETCH_ASSOC)){
                if ($linha['cpf']==="Não informado") {
                    $internos[$x]=array('cpf'=>$linha['cpf'],'nome'=>$linha['nome'],'id'=>$linha['id_interno']);
                }
                else{
